...
const DownLoadImage = async (NameBreed) => {
...
function SearchView() {
    const [SearchResoults, SetSearchResoults] = React.useState(0)
    const [Name, SetName] = React.useState("")
    const location = useLocation()
    const StartSearch = React.useCallback((ev) => {
        ev.preventDefault()
        const data = new FormData(ev.target).get("SearchName");
        if (data != undefined && data.trim().length != 0)
            DownLoadImage(data).then(
                resp => {
                    SetName(data)
                    SetSearchResoults(resp)
                }
            )
    }, [])
    ...
    React.useEffect(() => {
        ...
        if (urlParams.get("name") != undefined && urlParams.get("name") != null) 
            DownLoadImage(urlParams.get("name")).then(
                resp => {
    ...
    return (
        <div className="HomeScreen_wrapper">
            {
                SearchResoults === 0 && <a className="HomeScreen_wrapper_title">Szukaj a znajdziesz :)</a>
            }
            <form onSubmit={StartSearch}>
                <fieldset>
                    <legend>
                        Wpisz rasę, którą szukasz
                    </legend>
                    <input type="text" name="SearchName" placeholder="np. Bulldog angielski"/>
                </fieldset>
                <input type="submit" value="szukaj"/>
            </form>
            <div className="SearchView_resoults">
                <div>
                    {
                        (SearchResoults === null || SearchResoults === 0) && 
                    ...
                    {
                        (SearchResoults != null && SearchResoults != 0) &&
                        <img src={SearchResoults} alt="Dog"/>
                    }
                </div>
                <p>
                    {
                        (SearchResoults != null && SearchResoults != 0) &&  <b style={{color: "white", textTransform: "capitalize"}}>{Name}</b>
                    }
                    <br></br>
                   { SearchResoults == null && "Ajajaj! Tej rasy nie ma jeszcze w naszej bazie. Poszukaj innej."}
                   { SearchResoults === 0 && "Tu wyświetlimy informacje o interesującym Cię popilu."}
                   {
                        (SearchResoults != null && SearchResoults != 0) && 
                        `
                        Ten pies to wierny i przyjacielski czworonożny towarzysz, który świetnie czuje się w roli rodzinnego przyjaciela. Dobrze dogaduje się z dziećmi i uwielbia się bawić wspólnie.

                        Jest łatwy w prowadzeniu, choć bywa uparty. Sprawdza się zarówno w małym mieszkaniu, jak i w domu z ogrodem.
                        
                        Wysokość w kłębie wynosi około 30–35 cm, a całkowita masa ciała to 22–25 kg. Jego sierść jest krótka, gęsta, lśniąca, o płowym, pręgowanym lub łaciastym umaszczeniu. Charakterystyczne cechy teriera psowatego to czujność, żywiołowość, oddanie, odwaga, łagodność, choć czasem może być uparty. W zależności od dnia może pokazywać różne oblicza swojej natury
                        `
                   }
                </p> 
            </div>
